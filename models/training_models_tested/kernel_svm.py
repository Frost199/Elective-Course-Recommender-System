...
def cm_analysis(y_true, y_pred, labels, filename, given_figsize):
    ...
    plt.title('Confusion Matrix for Kernel SVM (TEST SET) Model')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.savefig(filename)
    plt.show()
cm_analysis(y_test, y_pred, classifier.classes_, 'Confusion Matrix Kernel_svm_test.png', given_figsize=(19,19))
...
plt.title('Kernel SVM (TRAINING SET) Exam Scores and Predicted Score Category')

plt.show()
plt.savefig('Kernel_svm_train.png')
...
ax.legend(handles, ['[0-5]', '[10-15]', '[15-20]', '[20-25]', '[25-30]',
                    ...
                    '[75-80]', '[80-85]', '[85-90]', '[90-95]', '[95-100]'],
    framealpha=0.3, scatterpoints=1)
plt.title('Kernel SVM (TEST SET) Exam Scores and Predicted Score Category')

plt.show()
plt.savefig('Kernel_svm_test.png')
