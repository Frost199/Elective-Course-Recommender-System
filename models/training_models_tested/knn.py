...
for i in k_range:
    ...
    scores = cross_val_score(estimator=classifier, X=x, y=y, cv=30, scoring='accuracy')
...
def cm_analysis(y_true, y_pred, labels, filename, given_figsize):
    ...
    plt.title('Confusion Matrix for KNN (TEST SET) Model')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.savefig(filename)
    plt.show()
cm_analysis(y_test, y_pred, classifier.classes_, 'Confusion Matrix knn.png', given_figsize=(19,19))
...
plt.title('KNN (TRAINING SET) Exam Scores and Predicted Score Category')
...
ax.legend(handles, ['[0-5]', '[10-15]', '[15-20]', '[20-25]', '[25-30]',
                    ...
                    '[75-80]', '[80-85]', '[85-90]', '[90-95]', '[95-100]'],
    framealpha=0.3, scatterpoints=1)
plt.title('KNN (TEST SET) Exam Scores and Predicted Score Category')
